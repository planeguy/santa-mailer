...
const santaarray = santas.flatMap(s=>s.email).filter(m=>!!m).map(m=>m.toLowerCase());
const whitelist = santas.flatMap(s=>[s.email,...(Array.isArray(s.aka)?s.aka:[s.aka])]).filter(m=>!!m).map(m=>m.toLowerCase());
console.log(santaarray, whitelist);
